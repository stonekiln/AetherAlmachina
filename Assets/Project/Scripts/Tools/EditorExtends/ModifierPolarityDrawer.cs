...
namespace EditorExtends
{
    ...
    public class ModifierPolarityDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            List<KeyValuePair<string, Type>> types = TypeCache.GetTypesDerivedFrom(typeof(ModifierPolarity)).Where(t => !t.IsAbstract && !t.IsGenericType).OrderBy(t => t.Name).Select(t => new KeyValuePair<string, Type>(t.Name, t)).ToList();
            int selectIndex;
            bool isSelect;
